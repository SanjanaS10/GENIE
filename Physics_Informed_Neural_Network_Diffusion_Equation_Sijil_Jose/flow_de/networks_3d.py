...
def loss_function_physics(model: PhysicsNN, t_f: torch.Tensor, z : torch.Tensor) -> torch.Tensor:
    # Loss associated with the physics governing the model
    MSE_f = f(model, t_f, z).pow(2).sum(axis = 1)
...
def loss_function_physics_batched(model, T_all, Z_all, batch_size=2048):
    """
    Evaluates physics loss at all (T_all, Z_all) points using mini-batches.
    Returns tensor of shape [N], one loss per collocation point.
    """
    model.eval()
    physics_losses = []

    dataset = torch.utils.data.TensorDataset(T_all, Z_all)
    loader = torch.utils.data.DataLoader(dataset, batch_size=batch_size)

    for T_batch, Z_batch in loader:
        T_batch = T_batch.to(DEVICE)
        Z_batch = Z_batch.to(DEVICE)

        try:
            loss_batch = f(model, T_batch, Z_batch).pow(2).sum(dim=1)  # shape [batch_size]
            physics_losses.append(loss_batch.detach())
        except RuntimeError as e:
            print("OOM during physics evaluation, consider reducing batch_size")
            raise e
        finally:
            del T_batch, Z_batch, loss_batch
            torch.cuda.empty_cache()
    return torch.cat(physics_losses, dim=0)  # shape [N]
    ...
    for i, (z_size, lr) in enumerate(zip(z_sizes, lrs)):
        ...
        for epoch in range(epoch):
            ...
            if (epoch % 1000 == 0) and (epoch > 1):
              ...
              T_sample = torch.rand(10000, 1, dtype =torch.float32)
              Z_sample = torch.rand(10000, z_subset.shape[1], dtype = torch.float32)*5# or domain-specific sampler


              del colloc_dataset, colloc_loader
              del T_batch, Z_batch
              torch.cuda.empty_cache()

              pointwise_losses = loss_function_physics_batched(model, T_sample, Z_sample, batch_size=256)

              #print(pointwise_losses.shape)
              assert pointwise_losses.shape[0] == T_sample.shape[0]
              # Take top-k highest loss points
              topk = 5000
              top_indices = torch.topk(pointwise_losses.squeeze(), topk).indices.to('cpu')

              T_flat = T_sample[top_indices]
              Z_flat = Z_sample[top_indices]
              del T_sample, Z_sample
              # Update collocation loader
              colloc_dataset = TensorDataset(T_flat, Z_flat)
              colloc_loader = DataLoader(colloc_dataset, batch_size=5000, shuffle=True)
              print(f"[Adaptive Sampling] Replaced collocation set with top-{topk} high-residual points.")
